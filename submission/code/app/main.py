...
from app.utils.audio import decode_base64_mp3_to_pcm
...
async def health():
    ...
    return {"status": "healthy", "model_loaded": model_path.exists(), "version": "1.0"}
...
async def info():
    ...
    try:
        ...
        p = Path(__file__).resolve().parents[1] / "results" / "training_report.json"
        if p.exists():
            with open(p, "r", encoding="utf-8") as f:
                obj = _json.load(f)
            acc = float(obj.get("accuracy_test", acc))
            auc = float(obj.get("roc_auc_test", auc))
            ece = float(obj.get("ece_test", ece))
        pb = Path(__file__).resolve().parents[1] / "results" / "benchmark_results.json"
        if pb.exists():
            with open(pb, "r", encoding="utf-8") as f:
                bm = _json.load(f)
            p95 = int(bm.get("p95_latency_ms", p95))
    except Exception:
        pass
    return {
        "model_version": "1.0",
        "language_support": "English",
        "model_metrics": {"accuracy": acc, "roc_auc": auc, "ece": ece, "latency_p95_ms": p95},
        "features": "9 acoustic features (language-agnostic design)",
        "note": "Model trained and optimized specifically for English audio"
    }
...
async def voice_detection(request: Request, voice_request: VoiceDetectionRequest, x_api_key: str = Header(default="")):
    ...
    try:
        ...
        return JSONResponse(status_code=400, content={"status": "error", "message": "Invalid audio format"})
